...
class AppSetting(Base):
    ...
    finance_can_unlock_customer: Mapped[bool] = mapped_column(
        Boolean, default=False, server_default="false"
    )
    # JSON list of customer IDs unlocked via unlock-all; used by relock-previous
    finance_lock_restore_ids: Mapped[str | None] = mapped_column(Text, nullable=True)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False
    )
