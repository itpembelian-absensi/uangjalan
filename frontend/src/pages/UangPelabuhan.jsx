  ...
  <>
    <div className="form-group">
      ...
      <input
        ...
        placeholder="Misal: Engkle, Double, Fuso, Tronton"
        value={values.name}
    ...
    <div className="form-group" style={{ marginBottom: 0 }}>
      <label className="form-label">Nominal Uang Pelabuhan (Rp)</label>
      <input
        type="number"
        className="form-input"
        min="0"
...
const UangPelabuhan = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      await apiFetch('/api/uang-pelabuhan', {
        method: 'POST',
  ...
  const handleEditSubmit = async (e) => {
    ...
    setError('');
    try {
      await apiFetch(`/api/uang-pelabuhan/${editId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDelete = async (row) => {
    if (!window.confirm(`Hapus Master Uang Pelabuhan "${row.name}"?`)) return;
    setError('');
    try {
      await apiFetch(`/api/uang-pelabuhan/${row.id}`, { method: 'DELETE' });
      await fetchItems();
    } catch (err) {
      setError(err.message);
    }
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1>Master Uang Pelabuhan</h1>
          <p>Kelola nominal uang pelabuhan per jenis kendaraan.</p>
        </div>
      </div>
      ...
      <div className="grid-cols-3">
        ...
        <div style={{ gridColumn: 'span 1' }}>
          <GlassCard title="Tambah Uang Pelabuhan">
            <form onSubmit={handleSubmit}>
              ...
              <button type="submit" className="btn btn-primary" style={{ width: '100%', marginTop: '1rem' }} disabled={saving}>
                <Plus size={18} /> {saving ? 'Menyimpan...' : 'Simpan Uang Pelabuhan'}
              </button>
            </form>
          </GlassCard>
        </div>
                ...
                {items.length === 0 && (
                  <tr>
                    <td colSpan={canWrite ? 4 : 3} style={{ textAlign: 'center', opacity: 0.5, padding: '2rem' }}>
                      Belum ada data Master Uang Pelabuhan
                    </td>
                  </tr>
                )}
              </tbody>
