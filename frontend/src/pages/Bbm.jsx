  ...
  <>
    <div className="form-group">
      ...
      <input
        ...
        placeholder="Misal: Solar, Pertamax, Pertamina Dex"
        value={values.name}
    ...
    <div className="form-group" style={{ marginBottom: 0 }}>
      <label className="form-label">Harga (Rp/Liter)</label>
      <input
        type="number"
        className="form-input"
        min="0"
...
const Bbm = () => {
  ...
  const handleDelete = async (row) => {
    if (!window.confirm(`Hapus BBM "${row.name}"?`)) return;
    setError('');
    try {
      await apiFetch(`/api/bbm/${row.id}`, { method: 'DELETE' });
      await fetchItems();
    } catch (err) {
      setError(err.message);
    }
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1>Master BBM</h1>
          <p>Kelola jenis bahan bakar dan harga per liter.</p>
        </div>
      </div>
          ...
          <GlassCard title="Tambah BBM">
            <form onSubmit={handleSubmit}>
              ...
              <button type="submit" className="btn btn-primary" style={{ width: '100%', marginTop: '1rem' }} disabled={saving}>
                <Plus size={18} /> {saving ? 'Menyimpan...' : 'Simpan BBM'}
              </button>
            </form>
          </GlassCard>
        </div>
            ...
            <table className="glass-table">
              <thead>
                <tr>
                  <th>ID</th>
                  <th>Nama BBM</th>
                  <th style={{ textAlign: 'right' }}>Harga/Liter</th>
              ...
              <tbody>
                {items.map((row) => (
                  <tr key={row.id}>
                    ...
                    <td style={{ textAlign: 'right' }}>{formatIDR(row.price)}</td>
