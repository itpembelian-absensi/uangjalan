...
SQL
  ...
  log "Verify Karawaci/Bitung + JORR Kayu Besar rates"
  docker compose exec -T db psql -U postgres -d uang_pengiriman -c \
    "SELECT destination_name, gol23, gol45 FROM toll_sections WHERE (name='Tangerang - Merak' AND destination_name IN ('Karawaci','Bitung')) OR (name='JORR' AND origin_name='Kayu Besar') ORDER BY sort_order;"
