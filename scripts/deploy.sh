...
SQL
  ...
  # ---- Seed data: JORR Kayu Besar → Jati Asih / Priok ----
  docker compose exec -T db psql -U postgres -d uang_pengiriman -v ON_ERROR_STOP=0 < scripts/seed_jorr_kayu_besar.sql

  # ---- Seed data: Kayu Besar → Ulujami → Pondok Aren (Bintaro) ----
  docker compose exec -T db psql -U postgres -d uang_pengiriman -v ON_ERROR_STOP=0 < scripts/seed_kayu_besar_bintaro.sql
  ...
  docker compose exec -T backend python add_jorr_kayu_besar_tolls.py || \
    docker compose run --rm --no-deps backend python add_jorr_kayu_besar_tolls.py || true
  docker compose exec -T backend python add_kayu_besar_bintaro_tolls.py || \
    docker compose run --rm --no-deps backend python add_kayu_besar_bintaro_tolls.py || true
  ...
  docker compose exec -T db psql -U postgres -d uang_pengiriman -c \
    "SELECT name, origin_name, destination_name, gol23 FROM toll_sections WHERE (name='Tangerang - Merak' AND destination_name IN ('Karawaci','Bitung')) OR (name='JORR' AND origin_name='Kayu Besar') OR (name='Pondok Aren-Bintaro Viaduct-Ulujami' AND origin_name='Ulujami') OR name='Bocimi' OR (origin_name='Cikunir' AND name ILIKE '%Cikampek%') ORDER BY sort_order;"
